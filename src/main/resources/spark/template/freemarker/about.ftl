...
<@page.page title="About XMPP Status Site" about=true>
    ...
    <p>This is the status site for the listed XMPP servers.  This site shows whether each server is up and accepting connections
       form clients and other servers.  It also shows the historical uptime of the servers.</p>

    <p>This service will connect to one of the <i>primary</i> domains, as a client, via a standerd JID to the C2S port 5222.
       It then will send a <code>ping</code> request to each of the <b>remote</b> domains (both the <i>Remote Domains</i> and the
    ...
    <p>This site checks the XMPP service every 10 minutes.</p>
